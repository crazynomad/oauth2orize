...
module.exports = function(options) {
  ...
  return function errorHandler(err, req, res, next) {
    if (mode == 'direct') {
      //New user will output a "new" field
      if(err.code && parseInt(err.code) == 640 && err.token){
          res.setHeader('Content-Type', 'application/json');
          var new_user_token = {};
          new_user_token.access_token = err.token;
          new_user_token.token_type = "Bearer";
          new_user_token.new = 1;
          return res.end(JSON.stringify(new_user_token));
      }
      ...
      res.setHeader('Content-Type', 'application/json');
      if(err.code)
        res.setHeader('BackFire-Code',err.code);
      if(err.message)
        res.setHeader('BackFire-Message',err.message);
